<template>
    <div class="lang-select df jcc aic">
        <ul class="lang-select__list df fdc aic"
            :style="{ height: `${languages.length * 100}%`, top: `${-100 * activeIndex}%` }">
            <li
                    :class="`lang-select__option df aic jcc ${index === activeIndex && 'active'}`"
        ...
        <span class="lang-select__controls df fdc aic jcsb">
            <v-icon name="keyboard_arrow_up" @click="go(-1)" />
            <v-icon name="keyboard_arrow_down" @click="go(1)" />
        </span>
    </div>
</template>
